...
[logp_confidence, yhat_confidence, res_confidence] = ...
    obs1_softmax_binary(r, infStates, ptrans_softmax);

%% get combined log likelihood of two response data modalities
logp = logp_binary + logp_reactionTime + logp_confidence;
logp_split = [logp_binary, logp_reactionTime, logp_confidence];

%% return predictions and responses for each response data modality
yhat = [yhat_binary, yhat_reactionTime, yhat_confidence];
res = [res_binary, res_reactionTime, res_confidence];

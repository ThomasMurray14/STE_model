function [pvec, pstruct] = obs3_gaussianRT_transp(r, ptrans)
...
% gaussian
pvec(1)     = tapas_sgm(ptrans(1), 1); % mu
pstruct.mu = pvec(1);

pvec(2)     = exp(ptrans(2));         % sigma0
pstruct.sigma0 = pvec(2);

pvec(3)     = exp(ptrans(3));         % gamma1
pstruct.gamma1 = pvec(3);

pvec(4)     = exp(ptrans(4));    % lambda1
pstruct.lambda1  = pvec(4);

pvec(5)     = exp(ptrans(5));    % sigma1 (noise)
pstruct.sigma1  = pvec(5);
